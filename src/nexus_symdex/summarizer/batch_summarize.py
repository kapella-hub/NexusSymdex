...
class BaseSummarizer:
    """Base class for AI-based batch summarization (Tier 2).

    Subclasses must implement ``_init_client`` and ``_summarize_one_batch``.
    """
    ...
    def _init_client(self):
        """Initialize the provider-specific client. Override in subclasses."""
        ...
        try:
            ...
            if os.environ.get("ANTHROPIC_API_KEY"):
                ...
                warnings.warn(
                    ...
                    "Install it with: pip install nexus-symdex[anthropic]",
                    stacklevel=2,
                )
            self.client = None

    def _summarize_one_batch(self, batch: list[Symbol]):
        """Summarize one batch of symbols via Anthropic API."""
        ...
        try:
            ...
            if os.environ.get("GOOGLE_API_KEY"):
                ...
                warnings.warn(
                    ...
                    "Install it with: pip install nexus-symdex[gemini]",
                    stacklevel=2,
                )
            self.client = None

    def _summarize_one_batch(self, batch: list[Symbol]):
        """Summarize one batch of symbols via Gemini API."""
        prompt = self._build_prompt(batch)
